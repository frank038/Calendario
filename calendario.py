...
CALSCALE:GREGORIAN\n""")
    ...
    def month_view(self, todata):
        ...
        DAYS_THIS_MONTH, DAYS_PREV_MONTH = self.num_days_month(datetime.datetime.now().date()) #TODAY)
        ...
        number_of_weeks_in_year = datetime.date(self.NUM_YEAR, 12, 21).isocalendar()[1]
        iidx = 0
        for i in range(6):
            fframe = Gtk.Frame()
            label = Gtk.Label(label=iidx+first_month_week)
            iidx += 1
            if iidx+first_month_week == number_of_weeks_in_year + 2:
                iidx = 0
                first_month_week = 1
            fframe.add(label)
    ...
    def num_days_month(self, TODAY):
        ...
        if month != 12:
            DAYS_THIS_MONT = (datetime.date(year, month+1, 1) - datetime.date(year, month, 1)).days
        else:
            DAYS_THIS_MONT = (datetime.date(year+1, 1, 1) - datetime.date(year, month, 1)).days
        
        if month == 2:
            DAYS_PREV_MONT = (datetime.date(year, 1, 1) - datetime.date(year-1, 12, 1)).days
        elif month == 1:
            DAYS_PREV_MONT = (datetime.date(year-1, 12, 1) - datetime.date(year-1, 11, 1)).days
        else:
            DAYS_PREV_MONT = (datetime.date(year, month-1, 1) - datetime.date(year, month-2, 1)).days
        
        
        return (DAYS_THIS_MONT, DAYS_PREV_MONT)
